...
window.__initSmoothScroll = function () {
  ...
  function scrollArray(elem, left, top) {
    ...
    // prevent queue bloat during rapid scrolling
    if (que.length > 10) {
      que = que.slice(-3);
    }
  ...
  function wheel(event) {
    ...
    // zooming event with CTRL - full reset so smooth scroll
    // works correctly after zoom changes page dimensions
    if (event.ctrlKey) {
      que = [];
      pending = false;
  ...
  function scheduleClearCache() {
    ...
    clearCacheTimer = setTimeout(function () {
      cacheX = cacheY = smoothBehaviorForElement = {};
    }, 1 * 1000);
  }
